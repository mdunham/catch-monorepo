...
function helper(obj, tokens, ctx, fnArgs) {
  ...
  if (isTokenFunctionCall(currentToken)) {
    return helper(obj[isArray(fnArgs[0]) ? 'apply' : 'call'](ctx, fnArgs[0]), tokens.slice(1), // clear context because consecutive fn calls execute in global context
    // e.g. `a.b()()`
    null, fnArgs.slice(1));
  } else if (isTokenArrayAccess(currentToken)) {
    return helper(obj[parseInt(currentToken.substr(1), 10)], tokens.slice(1), // lookahead two tokens for function calls
    isTokenFunctionCall(tokens[1]) ? obj : ctx, fnArgs);
  } else {
    return helper(obj[currentToken], tokens.slice(1), // lookahead two tokens for function calls
    isTokenFunctionCall(tokens[1]) ? obj : ctx, fnArgs);
  }
}
